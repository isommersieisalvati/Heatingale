...
const FederalElectionTurnout = () => {
  ...
  const data = [
    ...
    { year: 2025, turnout: 68.7 },
  ];
  const avg = d3.median(data, (d) => d.turnout);
  const min = d3.min(data, (d) => d.turnout);
  const max = d3.max(data, (d) => d.turnout);
  ...
  useEffect(() => {
    ...
    const rScale = d3
      ...
      .range([0, radius * 0.2, radius]); // 0–50% compressed, 50–100% proportional

    const axisGroup = g.append("g");

    // Define ticks to show
    const tickValues = [40, 50, 60, 70, 80];
    ...
    const colorScale = d3
      .scaleDiverging()
      .domain([min, avg, max])
      .interpolator(d3.interpolateRgbBasis(["#000080", "#eeeeee", "#1e570c"]));
    ...
    data.forEach((d, i) => {
      ...
      const currentDeg = startDeg + i * angleStep;
      const theta = angle(currentDeg); // angle in radians
      const barLength = rScale(d.turnout);

      const r1 = rScale(d.turnout); // outer radius
      if (r1 <= r0) return; // skip if below 50%

      const x1 = Math.cos(theta) * r0;
      const y1 = Math.sin(theta) * r0;
      const x2 = Math.cos(theta) * r1;
      const y2 = Math.sin(theta) * r1;
      ...
      g.append("line")
        ...
        .attr("x2", x2)
        .attr("y2", y2)
        .attr("stroke", colorScale(d.turnout))
        .attr("stroke-width", 5);

      g.append("text")
        .attr("x", Math.cos(theta) * (barLength + 15))
        .attr("y", Math.sin(theta) * (barLength + 15))
    ...
    [40, 50, 60, 70, 80].forEach((t) => {
      const r = rScale(t);
      refGroup
        .append("circle")
        .attr("r", r)
        .attr("stroke", "#bbb") // softer gray
        .attr("stroke-dasharray", "4, 2") // dashed line pattern
        .attr("fill", "none");
    });
  }, [data, width, height]);
    ...
    <div className="federal-election-turnout">
      <div className="federal-election-turnout-text">
        <div className="federal-election-turnout-title">
          Federal Election Voter Turnout <br /> (1867-2021)
        </div>
        <div className="federal-election-turnout-summary">
          <div>
            <span className="label">Average:</span> {avg}%
          </div>
          <div>
            <span className="label">Lowest:</span> {min}% in 1898
          </div>
          <div>
            <span className="label">Highest:</span> {max}% in 1958
          </div>
        </div>
        <div className="federal-voter-turnout-note">
          * This voter turnout is calculated based on the ratio of total ballots
          cast and the number of electors on list. The voter turnout as
          percentage of total population is much lower, between 8% to 52%.
        </div>
